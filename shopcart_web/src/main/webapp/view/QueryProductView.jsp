...
<html>
  ...
  <!-- Title -->
   ...
   <div class="c2"> 欢迎${sessionScope.username } </div>
 	<div class="c2"> 当前在线人数为 : ${applicationScope.count }</div>
  ...
  <form action="${path }/a/QueryByPriceAction" method="post">
       ...
       <td><b>Id</b></td>
        ...
        <c:forEach items="${list}" var="p">
        	<tr align="center">
	          ...
	          <td>${p.getPrice()}</td>     
	          <td align="center"><a href="${path }/a/AddCartItemAction?id=${p.getProductName()}"><img src="${path}/image/car_new.gif"/></a></td>
	   		</tr>
	    </c:forEach>   
   ...
   <c:if test="${currentPage!=1}">
     <a href="${path}/a/QueryProductAction?currentPage=${currentPage-1}">上一页</a>
    </c:if>
    &nbsp;
    ...
    <c:forEach begin="1" end="${totalPage}" var="currentPage">
    	<a href="${path}/a/QueryProductAction?currentPage=${currentPage}">${currentPage}</a>
    </c:forEach>
    
   <c:if test="${currentPage!=totalPage}">
    <a href="${path}/a/QueryProductAction?currentPage=${currentPage+1}">下一页</a>
    </c:if>
    &nbsp;
